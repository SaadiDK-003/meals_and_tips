    ...
    if (!empty($_FILES["recipe_img"]["name"])) {
        ...
        if (in_array($fileType, $allowTypes)) {
            ...
            if (move_uploaded_file($_FILES["recipe_img"]["tmp_name"], $targetFilePath)) {
                ...
                if ($recipe_Q):
                    $msg = json_encode(["status" => "d-block alert alert-success", "msg" => "Recipe added successfully."]);
                endif;
            } else {
                $msg = json_encode(["status" => "d-block alert alert-danger", "msg" => "Sorry, there was an error uploading your file."]);
            }
        } else {
            $msg = json_encode(["status" => "d-block alert alert-danger", "msg" => "Sorry, only JPG, JPEG, PNG & GIF files are allowed to upload."]);
        }
    }
    echo $msg;
endif;
